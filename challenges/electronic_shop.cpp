 ...
 */
int getMoneySpent(Vector keyboards, Vector drives, int max_budget) {
  /*
  ...
  for (auto itk = keyboards.rbegin(); itk != keyboards.rend(); ++itk) {
    for (auto itd = drives.rbegin(); itd != drives.rend(); ++itd) {
      sum = *itk + *itd;

      if (sum <= max_budget && sum > cur_best)
        cur_best = sum;
    }
  }
